...
- ホーム下部に5ボタン（左→右）: アプリ設定 / 履歴リスト / ホーム / 着信予定メールリスト（編集） / 留守電リスト
- 履歴リストは削除可能。ブックマークで消えないように固定可能
- 未来日時入力のクイックリスト/DatePickerはキーパッド「0」の左のボタン押下時のみ小ウィンドウで表示
...
- 不達→留守電: 指定時刻から一定時間応答が無い（アプリ未起動/タイムアウト等）→ 留守電リストに登録・バッジ加算 → ホーム下部の留守電ボタンからリストへ → 1回再生でリストから消え履歴へ移動
...
## 画面一覧
- ホーム: 中央の新規“発信”ボタン、次の予定、最近の履歴ダイジェスト。画面下部に5ボタン（設定/履歴/ホーム/着信予定メール/留守電）
...
- 着信予定メールリスト（編集）: 今後の“着信”予定一覧、編集/削除/並び替え
...
## 情報設計（IA/ナビゲーション）
- 下部5ボタンの固定ナビ: アプリ設定 / 履歴リスト / ホーム / 着信予定メールリスト（編集） / 留守電リスト
...
- 下部5ボタンナビ（アプリ設定/履歴リスト/ホーム/着信予定メールリスト/留守電リスト）
