...
概要：
通話後画面にて利用者の任意でボタンが追加されます。そちらを押すとgoogleChlome,またはSafariが開き、詳細設定画面にて入力されたURLに飛びます。ユーザーが入力していなかった場合は、ボタンそのものが現れません。

タスク機能

概要：
履歴の詳細画面にてチェックボックスで管理できるタスクが追加されます。こちらは詳細設定画面にてタスクを追加された際に現れます。また同時に制限時間を入力することができ、入力された場合はデータが履歴に遷移されたと同時にカウントダウンが作動する（履歴の詳細画面で確認可能）

こちらを把握した上で下記の詳細設定画面の概要を読み込んでください。
...
　- Stepperで時間・分・日を設定
　- 全削除で締切設定を自動で無効化し非表示
　- 選択肢に空白も用意して空白の場合は締切時刻なしとする(一部だけ空白の場合も同様)
